...
**已知问题**:
- `stats-calculator.ts` 文件过长（942 行），需拆分
- 多仓库场景下高级统计缺失

**建议优化**:
1. 拆分 `stats-calculator.ts` 为多个文件：
   - `basic-stats.ts`: 基础统计
   - `author-stats.ts`: 作者维度
   - `time-stats.ts`: 时间分布与模式
   - `quality-stats.ts`: 质量与协作指标
2. 支持多仓库高级统计（需合并原始 CommitRecord[]）
...
2. 在 `stats-calculator.ts` 中添加计算函数
...
├── stats-calculator.ts       # 统计计算（942 行，需拆分）
└── advanced/                 # 高级统计模块
